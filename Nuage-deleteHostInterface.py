...
"""
--- Object ---
Delete a host interface for a baremetal or a physical appliance

--- Usage ---
Run 'python Nuage-deleteHostInterface.py -h' for an overview
...
def main():
   ...
   """
    ...
    print ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > Host Interface %s is deleted : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, nuage_myHostIfName, HostId ))
    logger.warning ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > Host Interface %s is deleted : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, nuage_myHostIfName, HostId ))
