...
--- Usage ---
Run 'python Nuage-newL3Domain.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myDomainTemplate', required=False, help='The name of the Nuage L3Domain Template to which the L3DOMAIN will be instaciated', dest='nuage_myDomainTemplate', type=str)
    ...
    parser.add_argument('--nuage-leaking', required=False, help='Indicates if this domain is a leakable domain or not', dest='nuage_leaking', type=bool, default=False)
...
def main():
    ...
    nuage_leaking       = args.nuage_leaking
    ...
    myNewDomain = vsdk.NUDomain(name        = nuage_myDomain,
                                template_id = myDomainTemplate.id,
                                                ...
                                                leaking_enabled = nuage_leaking
                                               )
    print ('L3Domain %s is created.' % (myNewDomain.name))

    myEnterprise.create_child(myNewDomain)
    print ('L3Domain %s id: %s' % (myNewDomain.name, myNewDomain.domain_id))
    
    logger.warning('L3Domain %s id %s is created' % (myNewDomain.name, myNewDomain.domain_id))
