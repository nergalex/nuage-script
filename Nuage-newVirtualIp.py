...
"""
--- Object ---
Create a Virtual IP for a cluster of baremetals or a physical appliances

--- Usage ---
Run 'python Nuage-newVirtualIp.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myVipMac', required=True, help='Floating MAC of the Virtual IP', dest='nuage_myVipMac', type=str)
    parser.add_argument('--nuage-myVipIp', required=True, help='Floating IP of the Virtual IP', dest='nuage_myVipIp', type=str)
...
def main():
    ...
    nuage_myVipMac      = args.nuage_myVipMac
    nuage_myVipIp       = args.nuage_myVipIp
   ...
   """
    ...
    myNewVip = vsdk.NUVirtualIP(mac = nuage_myVipMac,
                                virtual_ip = nuage_myVipIp,
                                entity_scope = 'ENTERPRISE',
                                external_id = MyCmsName
                           )
    myVPort.create_child(myNewVip)
    
    print ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > VIP %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, myNewVip.virtual_ip, myNewVip.id ))
    logger.warning ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > VIP %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, myNewVip.virtual_ip, myNewVip.id ))
