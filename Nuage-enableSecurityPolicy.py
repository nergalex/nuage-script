...
def enable_policy_template(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_mySecurityPolicyName, nuage_myIngressOrEgress):
    ...
    if mySecurityPolicy.active == True:
        print ('WARNING: %s Security Policy Template %s is already active.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))
        logger.warning('%s Security Policy Template %s is already active.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))
        exit(0)

    mySecurityPolicy.active = True
    mySecurityPolicy.save()
    print ('%s Security Policy Template %s has been activated.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))

    logger.warning('%s Security Policy Template %s has been activated.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))
