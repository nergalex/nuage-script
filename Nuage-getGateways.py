...
"""
--- Object ---
Get a list of all Gateways, Ports and VLAN

--- Usage ---
Run 'python Nuage-getGateways.py -h' for an overview
...
def main():
    ...
    # Execute action
    logger.warning('>Gateway>Port_name>port_physical>Vlan_value>vlan_desc>vlan_id')
    
    for cur_gw in nc.gateways.get():
        print('\nGateway %s' % cur_gw.name)
        for cur_port in cur_gw.ports.get():
            print('  +-- Port %s - %s' % (cur_port.name, cur_port.physical_name))
            for cur_vlan in cur_port.vlans.get():
                print('  |  +-- Vlan %s - %s : %s status %s' % (cur_vlan.value, cur_vlan.description, cur_vlan.id, cur_vlan.status))
                logger.warning('>%s>%s>%s>%s>%s>%s' % (cur_gw.name, cur_port.name, cur_port.physical_name, cur_vlan.value, cur_vlan.description, cur_vlan.id))
