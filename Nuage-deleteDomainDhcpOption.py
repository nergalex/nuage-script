...
"""
--- Object ---
Delete a DHCP option attached to L3 DOMAIN

--- Usage ---
Run 'python Nuage-deleteDomainDhcpOption.py -h' for an overview
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="Delete a Subnet.")
...
def main():
    ...
    for cur_dhcp_option in myDomain.dhcp_options.get():
        if cur_dhcp_option.actual_type == nuage_myDhcpType:
            print ('|  +-- DHCP type %s value %s' % (cur_dhcp_option.actual_type, cur_dhcp_option.actual_values))
            DhcpOptionId = cur_dhcp_option.id
            cur_dhcp_option.delete()
            print ('DHCP type %s with id %s is deleted' % (cur_dhcp_option.actual_type, DhcpOptionId))
            logger.warning('DHCP type %s with id %s is deleted' % (cur_dhcp_option.actual_type, DhcpOptionId))
        else :
            print ('DHCP type %s not found' % (nuage_myDhcpType))
