...
"""
--- Object ---
Get a list of all Ports and VLAN for a given gateway

--- Usage ---
Run 'python Nuage-getPorts.py -h' for an overview
...
def main():
    ...
    for cur_port in myGateway.ports.get():
        print('  +-- Port %s - %s' % (cur_port.name, cur_port.physical_name))
        for cur_vlan in cur_port.vlans.get():
            print('  |  +-- Vlan %s - %s' % (cur_vlan.value, cur_vlan.description))  
        print('  |')
