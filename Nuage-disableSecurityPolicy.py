...
def main():
    ...
    if nuage_myTemplateOrDomain == "Template":
        enable_policy_template(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_mySecurityPolicyName, nuage_myIngressOrEgress)
    elif nuage_myTemplateOrDomain == "Domain":
        enable_policy_domain(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_mySecurityPolicyName, nuage_myIngressOrEgress)

    # TODO : Other options, security rules

def enable_policy_template(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_mySecurityPolicyName, nuage_myIngressOrEgress):
    ...
    if mySecurityPolicy.active == False:
        print ('WARNING: %s Security Policy Template %s is already disabled.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))
        logger.warning('%s Security Policy Template %s is already disabled.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))
        exit(0)

    mySecurityPolicy.active = False
    mySecurityPolicy.save()
    print ('%s Security Policy Template %s has been disabled.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))

    logger.warning('%s Security Policy Template %s has been disabled.' % (nuage_myIngressOrEgress, mySecurityPolicy.name))

def enable_policy_domain(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_mySecurityPolicyName, nuage_myIngressOrEgress):
    myFilter = "name == \"" + nuage_myDomain +"\""
