...
--- Usage ---
Run 'python Nuage-newSubnet.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-mySubnetGateway', required=False, help='The dVRS gateway of the Nuage Subnet to which the VM (baremetal or Appliance) should be connected', dest='nuage_mySubnetGateway', type=str)   
...
def main():
    ...
    nuage_mySubnetGateway = args.nuage_mySubnetGateway
    ...
    print ('Subnet %s is created.' % (myNewSubnet.name))

    myZone.create_child(myNewSubnet)
    print ('Enterprise %s > Domain %s > Zone %s > Subnet %s id: %s' % (myEnterprise.name, myDomain.name, myZone.name, myNewSubnet.name, myNewSubnet.id))
    
    logger.warning ('Enterprise %s > Domain %s > Zone %s > Subnet %s id: %s' % (myEnterprise.name, myDomain.name, myZone.name, myNewSubnet.name, myNewSubnet.id))
