...
--- Usage ---
Run 'python Nuage-newPolicyGroup.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myDescription', required=False, help='Description for this Policy Group', dest='nuage_myDescription', type=str)
...
def main():
    ...
    if nuage_myTemplateOrDomain == "Template":
        create_policy_template(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_myPolicyGroupName, nuage_myDescription, MyCmsName)
    elif nuage_myTemplateOrDomain == "Domain":
        create_policy_domain(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_myPolicyGroupName, nuage_myDescription, MyCmsName)

    # TODO : Other options, security rules


def create_policy_template(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_myPolicyGroupName, nuage_myDescription, MyCmsName):
    ...
    # Create Policy Group
    myNewPolicyGroup = vsdk.NUPolicyGroupTemplate(name = nuage_myPolicyGroupName,
                                                  description = nuage_myDescription,
                                                  external_id = MyCmsName
                                                 )
    print ('Policy Group Template %s is created.' % (myNewPolicyGroup.name))

    # Attach Policy Group to its domain
    myDomainTemplate.create_child(myNewPolicyGroup)
    print('%s Policy Group Template id: %s' % (myNewPolicyGroup.name, myNewPolicyGroup.id))
    logger.warning('%s Policy Group Template id: %s' % (myNewPolicyGroup.name, myNewPolicyGroup.id))


def create_policy_domain(logger, myEnterprise, nuage_myDomain, nuage_myL2OrL3, nuage_myPolicyGroupName, nuage_myDescription, MyCmsName):
    myFilter = "name == \"" + nuage_myDomain +"\""
    ...
    # Create Policy Group
    myNewPolicyGroup = vsdk.NUPolicyGroup(name = nuage_myPolicyGroupName, 
                                                  description = nuage_myDescription,
                                                  external_id = MyCmsName
                                                 )
    print ('Policy Group %s is created.' % (myNewPolicyGroup.name))

    # Attach Policy Group to its domain
    myDomain.create_child(myNewPolicyGroup)
    print('%s Policy Group id: %s' % (myNewPolicyGroup.name, myNewPolicyGroup.id))
    logger.warning('%s Policy Group id: %s' % (myNewPolicyGroup.name, myNewPolicyGroup.id))
