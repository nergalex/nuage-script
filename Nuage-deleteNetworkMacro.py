...
"""
--- Object ---
Delete a NetworkMacro

--- Usage ---
Run 'python Nuage-deleteNetworkMacro.py -h' for an overview
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="Delete a NetworkMacro.")
    ...
    parser.add_argument('--nuage-myNetworkMacroName', required=False, help='The name of the Nuage NetworkMacro that should be deleted', dest='nuage_myNetworkMacroName', type=str)
...
def main():
    ...
    myFilter = "name == \"" + nuage_myNetworkMacroName +"\""
    myNetworkMacro = myEnterprise.enterprise_networks.get_first(filter = myFilter)
    
    NetworkMacroId = myNetworkMacro.id
    print ('NetworkMacro %s id :%s' % (myNetworkMacro.name, myNetworkMacro.id))
    
    myNetworkMacro.delete()
    print ('NetworkMacro %s with id %s is deleted' % (nuage_myNetworkMacroName, NetworkMacroId))
    logger.warning('NetworkMacro %s with id %s is deleted' % (nuage_myNetworkMacroName, NetworkMacroId))
