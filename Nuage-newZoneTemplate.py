...
--- Usage ---
Run 'python Nuage-newZoneTemplate.py -h' for an overview
...
python Nuage-newZoneTemplate.py   --log-file Nuage-log.txt --nuage-organization TEST --nuage-host 127.1.1.2 --nuage-port 8443 --nuage-password test2018 --nuage-user admin-test --nuage-myEnterprise TEST --nuage-myDomain L3_PXY-RBD_FRT --nuage-myZone Interco --nuage-myZoneDesc "Interconnexion dVRS avec ADC"
...
def get_args():
    ...
    parser.add_argument('--nuage-myZoneDesc', required=False, help='The Description of the Nuage Zone to which the VM (baremetal or Appliance) should be connected', dest='nuage_myZoneDesc', type=str)
...
def main():
    ...
    nuage_myZoneDesc    = args.nuage_myZoneDesc
    ...
    myNewZoneTemplate = vsdk.NUZoneTemplate(name = nuage_myZone,
                                                dpi = 'DISABLED',
                                                ip_type = 'IPV4',
                                                description = nuage_myZoneDesc,
                                                encryption = 'INHERITED',
                                                entity_scope = 'ENTERPRISE',
                                                multicast = 'DISABLED',
                                                external_id = MyCmsName
                                               )
    print ('Zone Template %s is created.' % (myNewZoneTemplate.name))

    myDomainTemplate.create_child(myNewZoneTemplate)
    print ('Zone Template %s id: %s' % (myNewZoneTemplate.name, myNewZoneTemplate.id))
    
    logger.warning('Zone Template %s id %s is created' % (myNewZoneTemplate.name, myNewZoneTemplate.id))
