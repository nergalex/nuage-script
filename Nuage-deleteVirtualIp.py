...
"""
--- Object ---
Delete a Virtual IP for a cluster of baremetals or a physical appliances

--- Usage ---
Run 'python Nuage-deleteVirtualIp.py -h' for an overview
...
def main():
   ...
   """
    ...
    """    
    myFilter = "virtual_ip == \"" + nuage_myVipIp +"\""
    print ('myFilter : %s' % myFilter)

    nuage_myVipMac = 'f6:15:63:2c:70:01'
    myFilter = "mac == \"" + nuage_myVipMac +"\""
    print ('myFilter : %s' % myFilter)

    myVip = myVPort.virtual_ips.get_first(filter = myFilter)
    VipId = myVip.id
    myVip.delete()
    """
    
    for cur_Vip in myVPort.virtual_ips.get():
        if (cur_Vip.virtual_ip == nuage_myVipIp):
            VipId = cur_Vip.id
            cur_Vip.delete()
            
    print ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > VIP %s is deleted : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, nuage_myVipIp, VipId ))
    logger.warning ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > VIP %s is deleted : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, nuage_myVipIp, VipId ))
