...
"""
--- Object ---
Delete a NetworkMacroGroup

--- Usage ---
Run 'python Nuage-deleteNetworkMacroGroup.py -h' for an overview
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="Delete a NetworkMacroGroup.")
    ...
    parser.add_argument('--nuage-myNetworkMacroGroupName', required=False, help='The name of the Nuage NetworkMacroGroup that should be deleted', dest='nuage_myNetworkMacroGroupName', type=str)
...
def main():
    ...
    nuage-myNetworkMacroGroupName  = 'TEST'
    """
    ...
    myFilter = "name == \"" + nuage_myNetworkMacroGroupName +"\""
    myNetworkMacroGroup = myEnterprise.network_macro_groups.get_first(filter = myFilter)
    
    NetworkMacroGroupId = myNetworkMacroGroup.id
    print ('NetworkMacroGroup %s id :%s' % (myNetworkMacroGroup.name, myNetworkMacroGroup.id))
    
    myNetworkMacroGroup.delete()
    print ('NetworkMacroGroup %s with id %s is deleted' % (nuage_myNetworkMacroGroupName, NetworkMacroGroupId))
    logger.warning('NetworkMacroGroup %s with id %s is deleted' % (nuage_myNetworkMacroGroupName, NetworkMacroGroupId))
