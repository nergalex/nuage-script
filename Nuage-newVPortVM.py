...
"""
--- Object ---
Create a vPort for a Virtual Machine

--- Usage ---
Run 'python Nuage-newVPortVM.py -h' for an overview
...
def main():
    ...
    print ('vPortVM %s is created.' % (myNewVPort.name))

    mySubnet.create_child(myNewVPort)
    print ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPortVM %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myNewVPort.name, myNewVPort.id ))
    logger.warning ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPortVM %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myNewVPort.name, myNewVPort.id ))
