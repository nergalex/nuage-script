...
"""
--- Object ---
Create a L2 Domain template

--- Usage ---
Run 'python Nuage-newL2DomainTemplate.py -h' for an overview
...
python Nuage-newL2DomainTemplate.py   --log-file Nuage-log.txt --nuage-organization TEST --nuage-host 127.1.1.2 --nuage-port 8443 --nuage-password test2018 --nuage-user admin-test --nuage-myEnterprise TEST --nuage-myDomain L2_PXY-RBD_BCK --nuage-myDomainDesc "DMZ d hebergement des interfaces Back des Proxy ou Rebond et dont la gateway est le composant ADC"
...
def get_args():
    ...
    parser.add_argument('--nuage-myDomain', required=True, help='The name of the Nuage L2Domain to which the VM (baremetal or Appliance) should be connected', dest='nuage_myDomain', type=str)
    parser.add_argument('--nuage-myDomainDesc', required=True, help='The Description of the Nuage L2Domain to which the VM (baremetal or Appliance) should be connected', dest='nuage_myDomainDesc', type=str)
...
def main():
    ...
    myNewDomainTemplate = vsdk.NUL2DomainTemplate(name = nuage_myDomain,
                                                dhcp_managed = False,
                                                dpi = 'DISABLED',
                                                ip_type = 'IPV4',
                                                # gateway = '192.168.0.1',
    ...
    print ('Template L2Domain %s is created.' % (myNewDomainTemplate.name))

    myEnterprise.create_child(myNewDomainTemplate)
    print ('Template L2Domain %s id: %s' % (myNewDomainTemplate.name, myNewDomainTemplate.id))
    
    logger.warning('Template L2Domain %s id %s is created' % (myNewDomainTemplate.name, myNewDomainTemplate.id))
