...
--- Usage ---
Run 'python Nuage-deleteSubnetTemplate.py -h' for an overview
...
def main():
    ...
    mySubnetTemplate = myZoneTemplate.subnet_templates.get_first(filter = myFilter)
    
    SubnetId = mySubnetTemplate.id
    print ('Subnet Template %s id :%s' % (mySubnetTemplate.name, mySubnetTemplate.id))
    
    mySubnetTemplate.delete()
    print ('Subnet Template %s with id %s is deleted' % (nuage_mySubnetName, SubnetId))
    logger.warning('Subnet Template %s with id %s is deleted' % (nuage_mySubnetName, SubnetId))
