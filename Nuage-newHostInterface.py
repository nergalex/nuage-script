...
"""
--- Object ---
Create a host interface for a baremetal or a physical appliance

--- Usage ---
Run 'python Nuage-newHostInterface.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myHostIfMac', required=True, help='Static MAC of the appliance interface', dest='nuage_myHostIfMac', type=str)
    parser.add_argument('--nuage-myHostIfIp', required=True, help='Static IP of the appliance interface', dest='nuage_myHostIfIp', type=str)
...
def main():
    ...
    nuage_myHostIfMac   = args.nuage_myHostIfMac
    nuage_myHostIfIp    = args.nuage_myHostIfIp
   ...
   """
    ...
    myVPort = mySubnet.vports.get_first(filter = myFilter)
    print ('|   |   |   |')
    print ('|   |   |   +-- vPort : %s - %s' % (myVPort.name, myVPort.description))
    
    myNewHostInterface = vsdk.NUHostInterface(mac = nuage_myHostIfMac,
                                ip_address = nuage_myHostIfIp,
                                name = nuage_myHostIfName,
                                entity_scope = 'ENTERPRISE'
                           )
    myVPort.create_child(myNewHostInterface)
    print ('|   |   |   |   +-- Host Interface - vport_name: %s' % (myNewHostInterface.vport_name))
    print ('|   |   |   |   |   +-- gateway: %s' % (myNewHostInterface.gateway))
    print ('|   |   |   |   |   +-- netmask: %s' % (myNewHostInterface.netmask))
    print ('|   |   |   |   |   +-- network_name: %s' % (myNewHostInterface.network_name))
    print ('|   |   |   |   |   +-- tier_id: %s' % (myNewHostInterface.tier_id))
    print ('|   |   |   |   |   +-- domain_name: %s' % (myNewHostInterface.domain_name))
    print ('|   |   |   |   |   +-- zone_name: %s' % (myNewHostInterface.zone_name))

    print ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > Host Interface %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, myNewHostInterface.name, myNewHostInterface.id ))
    logger.warning ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > Host Interface %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, myNewHostInterface.name, myNewHostInterface.id ))
