...
"""
--- Object ---
Set DHCP option for a domain 

--- Usage ---
Run 'python Nuage-setDomainDhcpOption.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myDhcpType', required=True, help='DHCP type', dest='nuage_myDhcpType', type=int)
    parser.add_argument('--nuage-myDhcpValue', required=True, help='DHCP value', dest='nuage_myDhcpValue', type=str)
...
def main():
    ...
    nuage_myDhcpType    = args.nuage_myDhcpType
    nuage_myDhcpValue   = args.nuage_myDhcpValue
    ...
    # Execute action
    if nuage_myDhcpType in [15]:
        DhcpValueList = [nuage_myDhcpValue]
    else:
        DhcpValueList = []
        for curr_DhcpValue in nuage_myDhcpValue.split("-"):
            DhcpValueList.append(curr_DhcpValue)
    ...
    print('DhcpValueList : %s' % DhcpValueList)
    myNewDhcpOption = vsdk.NUDHCPOption(
                                        actual_type = nuage_myDhcpType,
                                        actual_values = DhcpValueList,
                                        external_id = MyCmsName
                                        )

    myDomain.create_child(myNewDhcpOption)
    
    print ('Domain %s > DHCP option %s value %s created.' % (myDomain.name, myNewDhcpOption.actual_type, myNewDhcpOption.actual_values))

    logger.warning ('Enterprise %s > Domain %s > DHCP option %s value %s' % (myEnterprise.name, myDomain.name, myNewDhcpOption.actual_type, myNewDhcpOption.actual_values))
