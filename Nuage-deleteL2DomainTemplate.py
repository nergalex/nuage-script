...
"""
--- Object ---
Delete a L2 Domain template

--- Usage ---
Run 'python Nuage-deleteL2DomainTemplate.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myDomain', required=False, help='The name of the Nuage L2Domain to which the VM (baremetal or Appliance) should be connected', dest='nuage_myDomain', type=str)
...
def main():
    ...
    DomainId = myDomainTemplate.id
    print ('Template %s id :%s' % (myDomainTemplate.name, myDomainTemplate.id))
    myDomainTemplate.delete()
    print ('Template %s with id %s is deleted' % (nuage_myDomain, DomainId))
    logger.warning('Template %s with id %s is deleted' % (nuage_myDomain, DomainId))
