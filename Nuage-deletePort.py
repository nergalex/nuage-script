...
"""
--- Object ---
Delete a Port of a Gateway

--- Usage ---
Run 'python Nuage-deletePort.py -h' for an overview
...
def main():
    ...
    portId = myPort.id
    print ('Port %s id :%s' % (myPort.name, myPort.id))
    
    myPort.delete()
    print ('Port %s with id %s is deleted' % (nuage_myPortName, portId))
    logger.warning('Port %s with id %s is deleted' % (nuage_myPortName, portId))
