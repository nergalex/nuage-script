...
--- Usage ---
Run 'python Nuage-deleteVM.py -h' for an overview
...
def main():
    ...
    # Execute action
    myFilter = "name == \"" + nuage_myVMname +"\""
    myVM = nc.vms.get_first(filter = myFilter)
    myVM.delete()

    print ('VM %s is deleted' % (nuage_myVMname))
    logger.warning ('VM %s is deleted' % (nuage_myVMname))
