...
"""
--- Object ---
Create a bridge interface for a baremetal or a physical appliance hosted into a L2 DOMAIN

--- Usage ---
Run 'python Nuage-newL2BridgeInterface.py -h' for an overview
...
def main():
   ...
   """
    ...
    print ('Entreprise %s > Domain %s > vPort %s > Host Interface %s is created : %s' % (myEnterprise.name, myDomain.name, myVPort.name, myNewBridgeInterface.name, myNewBridgeInterface.id ))
    logger.warning ('Entreprise %s > Domain %s > vPort %s > Host Interface %s is created : %s' % (myEnterprise.name, myDomain.name, myVPort.name, myNewBridgeInterface.name, myNewBridgeInterface.id ))
