...
--- Usage ---
Run 'python Nuage-newVlan.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myVlanDesc', required=False, help='Description of the VLAN on the port switch', dest='nuage_myVlanDesc', type=str)
...
def main():
    ...
    nuage_myVlanDesc    = args.nuage_myVlanDesc
    ...
    myNewVlan = vsdk.NUVLAN(value = nuage_myVlanValue,
                                                description = nuage_myVlanDesc,
                                                entity_scope = 'GLOBAL',
                                                type = 'ACCESS',
                                                permitted_action = 'ALL',
                                                external_id = MyCmsName
                                               )
    print ('Vlan %s is created.' % (myNewVlan.value))

    myPort.create_child(myNewVlan)
    print ('Vlan %s id: %s' % (myNewVlan.value, myNewVlan.id))
    
    logger.warning('Gateway %s > Port %s > Vlan %s id: %s' % (myGateway.name, myPort.name, myNewVlan.value, myNewVlan.id))
