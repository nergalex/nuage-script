...
"""
--- Object ---
Create a L3 Domain template

--- Usage ---
Run 'python Nuage-newL3DomainTemplate.py -h' for an overview
...
python Nuage-newL3DomainTemplate.py   --log-file Nuage-log.txt --nuage-organization TEST --nuage-host 127.1.1.2 --nuage-port 8443 --nuage-password test2018 --nuage-user admin-test --nuage-myEnterprise TEST --nuage-myDomain L3_PXY-RBD_BCK --nuage-myDomainDesc "DMZ d hebergement des interfaces Back des Proxy ou Rebond et dont la gateway est le composant ADC"
...
def get_args():
    ...
    parser.add_argument('--nuage-myDomainDesc', required=False, help='The Description of the Nuage L3Domain to which the VM (baremetal or Appliance) should be connected', dest='nuage_myDomainDesc', type=str)
...
def main():
    ...
    myNewDomainTemplate = vsdk.NUDomainTemplate(name = nuage_myDomain,
                                                dpi = 'DISABLED',
    ...
    print ('Template L3Domain %s is created.' % (myNewDomainTemplate.name))

    myEnterprise.create_child(myNewDomainTemplate)
    print ('Template L3Domain %s id: %s' % (myNewDomainTemplate.name, myNewDomainTemplate.id))
    
    logger.warning('Template L3Domain %s id %s is created' % (myNewDomainTemplate.name, myNewDomainTemplate.id))
