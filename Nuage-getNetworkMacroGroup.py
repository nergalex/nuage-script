...
"""
--- Object ---
Fetch a NetworkMacro

--- Usage ---
Run 'python Nuage-getNetworkMacro.py -h' for an overview
...
def get_args():
    ...
    parser.add_argument('--nuage-myNetworkMacroGroupName', required=False, help='The name of the Nuage Network Macro Group that should be created', dest='nuage_myNetworkMacroGroupName', type=str)
...
def main():
    ...
    if not myNetworkMacroGroup:
        print ('ERROR: Network Macro Group %s was not found.' % (nuage_myNetworkMacroGroupName))
        exit(1)

    print ('|   |   |')
    print ('|   |   +--- Network Macro Group %s :' % (myNetworkMacroGroup.name))
    print ('|   |        +--- id: %s' % (myNetworkMacroGroup.id))

    for network_macro in myNetworkMacroGroup.enterprise_networks.get():
        print ('|   |    |   +--- network macro: %s - %s/%s' % (network_macro.name, network_macro.address, network_macro.netmask))
