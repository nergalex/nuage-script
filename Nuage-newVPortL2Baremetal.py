...
"""
--- Object ---
Create a vPort for a baremetal or a physical appliance hosted into a L2 DOMAIN

--- Usage ---
Run 'python Nuage-newVPortL2Baremetal.py -h' for an overview
...
def main():
    ...
    print ('Entreprise %s > Domain %s > vPort %s is created : %s' % (myEnterprise.name, myDomain.name, myNewVPort.name, myNewVPort.id ))
    logger.warning ('Entreprise %s > Domain %s > vPort %s is created : %s' % (myEnterprise.name, myDomain.name, myNewVPort.name, myNewVPort.id ))
