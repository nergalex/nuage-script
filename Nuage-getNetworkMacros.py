...
def main():
    ...
    for cur_ent in nc.enterprises.get():
        print('Network Macros inside Enterprise %s' % cur_ent.name)
        for cur_networkMacro in cur_ent.enterprise_networks.get():
            print('|- Network Macro: %s %s %s' % (cur_networkMacro.name, cur_networkMacro.address, cur_networkMacro.netmask))
