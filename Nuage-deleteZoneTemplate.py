...
--- Usage ---
Run 'python Nuage-deleteZoneTemplate.py -h' for an overview
...
python Nuage-deleteZoneTemplate.py   --log-file Nuage-log.txt --nuage-organization TEST --nuage-host 127.1.1.2 --nuage-port 8443 --nuage-password test2018 --nuage-user admin-test --nuage-myEnterprise TEST --nuage-myDomain L3_SRV-3T --nuage-myZone Middle
...
def main():
    ...
    ZoneId = myZoneTemplate.id
    print ('Zone Template %s id :%s' % (myZoneTemplate.name, myZoneTemplate.id))
    myZoneTemplate.delete()
    print ('Zone Template %s with id %s is deleted' % (nuage_myZone, ZoneId))
    logger.warning('Zone Template %s with id %s is deleted' % (nuage_myZone, ZoneId))
