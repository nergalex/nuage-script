...
"""
--- Object ---
Create a bridge interface for a baremetal or a physical appliance
...
def main():
   ...
   """
    ...
    myNewBridgeInterface = vsdk.NUBridgeInterface(
                                name = nuage_myHostIfName,
                                entity_scope = 'ENTERPRISE'
                           )
    myVPort.create_child(myNewBridgeInterface)
    print ('|   |   |   |   +-- Host Interface - vport_name: %s' % (myNewBridgeInterface.vport_name))
    print ('|   |   |   |   |   +-- gateway: %s' % (myNewBridgeInterface.gateway))
    print ('|   |   |   |   |   +-- netmask: %s' % (myNewBridgeInterface.netmask))
    print ('|   |   |   |   |   +-- network_name: %s' % (myNewBridgeInterface.network_name))
    print ('|   |   |   |   |   +-- tier_id: %s' % (myNewBridgeInterface.tier_id))
    print ('|   |   |   |   |   +-- domain_name: %s' % (myNewBridgeInterface.domain_name))
    print ('|   |   |   |   |   +-- zone_name: %s' % (myNewBridgeInterface.zone_name))

    print ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > Host Interface %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, myNewBridgeInterface.name, myNewBridgeInterface.id ))
    logger.warning ('Entreprise %s > Domain %s > Zone %s > Subnet %s > vPort %s > Host Interface %s is created : %s' % (myEnterprise.name, myDomain.name, myZone.name, mySubnet.name, myVPort.name, myNewBridgeInterface.name, myNewBridgeInterface.id ))
