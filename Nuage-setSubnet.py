...
"""
--- Object ---
Set Subnet's parameters

--- Usage ---
Run 'python Nuage-setSubnet.py -h' for an overview
...
def main():
    ...
    print('mySubnet : %s' % mySubnet.name)
    mySubnet.address = nuage_mySubnetAddress
    mySubnet.netmask = nuage_mySubnetMask
    mySubnet.save()
    
    print ('Subnet %s %s is updated.' % (mySubnet.address, mySubnet.netmask))  
    logger.warning ('Domain %s > Zone %s > Subnet %s %s is updated.' % (myDomain.name, myZone.name, mySubnet.address, mySubnet.netmask))
